...
template <bool MT, bool MinSize>
[[noreturn]] void llvm_t<MT, MinSize>::die(const std::string &reason) {
...
static const char *name_of_global_for_offset(unsigned off) {
  ...
  TCGTemp *ts = &get_tcg_context()->temps[glb];
...
template <bool MT, bool MinSize>
llvm::Type *llvm_t<MT, MinSize>::PointerToWordType(void) {
  return llvm::PointerType::get(WordType(), 0);
}

template <bool MT, bool MinSize>
llvm::Type *llvm_t<MT, MinSize>::PPointerType(void) {
  return llvm::PointerType::get(PointerToWordType(), 0);
}

template <bool MT, bool MinSize>
llvm::Type *llvm_t<MT, MinSize>::VoidFunctionPointer(void) {
  llvm::FunctionType *FTy = llvm::FunctionType::get(VoidType(), false);
  return llvm::PointerType::get(FTy, 0);
}

template <bool MT, bool MinSize>
llvm::Constant *llvm_t<MT, MinSize>::BigWord(void) {
  //
...
//             % dummy_name
  ...
  for_each_if(
      ...
      [&](const Elf_Sym &Sym) {
        ...
        const function_t &f = function_at_address(Binary, Sym.st_value);
...
template <bool MT, bool MinSize>
void llvm_t<MT, MinSize>::DumpModule(const char *suffix) {
  //
...
#undef __THUNK
  ...
  if (IsCOFF) {
    if (EnvGlobal)
      EnvGlobal->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
    if (TraceGlobal)
    ...
    if (CallStackBeginGlobal)
      CallStackBeginGlobal->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
    if (GetEnvFunc)
      GetEnvFunc->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
    if (GetTraceFunc)
      GetTraceFunc->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
    if (GetCallStackFunc)
      GetCallStackFunc->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
    if (GetCallStackBeginFunc)
      GetCallStackBeginFunc->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
    ...
    if (auto *GV = Module->getGlobalVariable("__jove_trace", true)) {
      assert(!GV->hasInitializer());
      GV->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
    }

    if (auto *GV = Module->getGlobalVariable("__jove_trace_begin", true)) {
      assert(!GV->hasInitializer());
      GV->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
    }
  }
...
llvm_t<MT, MinSize>::declareHook(const hook_t &h, bool IsPreOrPost) {
...
int llvm_t<MT, MinSize>::LocateHooks(void) {
  ...
  for (const hook_t &h : HookArray) {
    ...
    for (dynamic_target_t IdxPair : (*it).second) {
      const function_t &f = function_of_target(IdxPair, jv);
...
llvm_t<MT, MinSize>::buildGlobalIFunc(const function_t &f,
  ...
  fillInFunctionBody(F, [&](auto &IRB) -> void {
    if (is_dynamic_target_valid(IdxPair) && IdxPair.first == BinaryIndex) {
      ...
      llvm::Value *SPPtr =
          BuildCPUStatePointer(IRB, GetEnv(IRB), tcg_stack_pointer_index);
...
template <bool MT, bool MinSize>
void llvm_t<MT, MinSize>::ExplodeFunctionArgs(const function_t &f,
                                              std::vector<unsigned> &glbv) {
...
template <bool MT, bool MinSize>
void llvm_t<MT, MinSize>::ExplodeFunctionRets(const function_t &f,
                                              std::vector<unsigned> &glbv) {
...
template <bool MT, bool MinSize>
llvm::FunctionType *llvm_t<MT, MinSize>::FunctionTypeOfArgsAndRets(tcg_global_set_t args,
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::CreateFunctions(void) {
  auto &Binary = jv.Binaries.at(BinaryIndex);
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::CreateFunctionTables(void) {
  for (binary_index_t BIdx = 0; BIdx < jv.Binaries.size(); ++BIdx) {
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::CreateFunctionTable(void) {
  auto &Binary = jv.Binaries.at(BinaryIndex);
  auto &ICFG = Binary.Analysis.ICFG;
...
static Value *getNaturalGEPWithOffset(jove::IRBuilderTy &,
                                      const DataLayout &,
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::CreateTLSModGlobal(void) {
...
template <bool MT, bool MinSize>
llvm::ArrayType *llvm_t<MT, MinSize>::TLSDescType(void) {
...
template <bool MT, bool MinSize>
llvm::GlobalVariable *llvm_t<MT, MinSize>::TLSDescGV(void) {
...
template <bool MT, bool MinSize>
llvm::Constant *llvm_t<MT, MinSize>::SymbolAddress(const elf::RelSymbol &RelSym) {
  assert(RelSym.Sym);
...
template <bool MT, bool MinSize>
llvm::Constant *llvm_t<MT, MinSize>::ImportFunction(llvm::StringRef Name) {
  llvm::FunctionType *FTy =
...
template <bool MT, bool MinSize>
llvm::Constant *llvm_t<MT, MinSize>::ImportFunctionByOrdinal(llvm::StringRef DLL,
                                                  uint32_t Ordinal) {
...
template <bool MT, bool MinSize>
llvm::Constant *llvm_t<MT, MinSize>::ImportedFunctionAddress(llvm::StringRef DLL,
                                                  uint32_t Ordinal,
                                                  llvm::StringRef Name,
                                                  uint64_t Addr) {
...
template <bool MT, bool MinSize>
void llvm_t<MT, MinSize>::elf_compute_irelative_relocation(IRBuilderTy &IRB,
  ...
  const function_t &resolver_f = function_at_address(Binary, resolverAddr);
...
template <bool MT, bool MinSize>
void llvm_t<MT, MinSize>::elf_compute_tpoff_relocation(IRBuilderTy &IRB,
...
template <bool MT, bool MinSize>
uint64_t llvm_t<MT, MinSize>::ExtractWordAtAddress(uint64_t Addr) {
  auto &Binary = jv.Binaries.at(BinaryIndex);
    ...
    for (const auto &pair : SectMap) {
      ...
      if (Sect.Name == ".rsrc") {
        if (_coff.rsrcSectIdx >= 0)
          throw std::runtime_error("multiple .rsrc sections in PE file?");
...
#undef __APINT_BYTE
  ...
  if (state.for_binary(Binary)._elf.OptionalDynSymRegion) {
    ...
    auto dynamic_table = [&](void) -> Elf_Dyn_Range {
      return state.for_binary(Binary)._elf.DynamicTable.template getAsArrayRef<Elf_Dyn>();
    };
    auto dynamic_symbols = [&](void) -> Elf_Sym_Range {
      return DynSymRegion.getAsArrayRef<Elf_Sym>();
    };
  ...
  // XXX this should go somewhere else
  {
    ...
    if (state.for_binary(binary)._elf.DynamicTable.Addr) {
      auto dynamic_table = [&](void) -> Elf_Dyn_Range {
        return state.for_binary(binary)
            ._elf.DynamicTable.template getAsArrayRef<Elf_Dyn>();
      };
    ...
    if (auto *F = Module->getFunction("_jove_get_init_fn"))
      fillInFunctionBody(F,
          [&](auto &IRB) {
            ...
            if (initFunctionAddr) {
              const function_t &initfn_f = function_at_address(Binary, initFunctionAddr);
    ...
    fillInFunctionBody(F,
        [&](auto &IRB) {
          ...
          if (libcEarlyInitAddr) {
            const function_t &f = function_at_address(Binary, libcEarlyInitAddr);
...
template <bool MT, bool MinSize>
std::pair<binary_index_t, std::pair<uint64_t, unsigned>>
llvm_t<MT, MinSize>::decipher_copy_relocation(const elf::RelSymbol &S) {
  assert(jv.Binaries.at(BinaryIndex).IsExecutable);
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::ProcessManualRelocations(void) {
  auto &Binary = jv.Binaries.at(BinaryIndex);
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::CreateCopyRelocationHack(void) {
  fillInFunctionBody(
      ...
      [&](auto &IRB) -> void {
        ...
        for (const auto &pair : CopyRelocMap) {
          ...
          if (pair.second.first < 3)
            WithColor::warning() << "CreateCopyRelocationHack\n";
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::CreateBinaryNamesTable(void) {
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::CreateNoAliasMetadata(void) {
  //
...
struct TranslateContext {
  ...
  TranslateContext(const function_t &f) : f(f) {
...
template <bool MT, bool MinSize>
llvm::AllocaInst *
llvm_t<MT, MinSize>::CreateAllocaForGlobal(TranslateContext &TC,
                                           IRBuilderTy &IRB, unsigned glb,
                                           bool InitializeFromEnv) {
  llvm::AllocaInst *res =
      IRB.CreateAlloca(TypeOfTCGGlobal(glb), nullptr,
                       std::string(get_tcg_context()->temps[glb].name));
...
template <bool MT, bool MinSize>
llvm::Value *llvm_t<MT, MinSize>::CPUStateGlobalPointer(IRBuilderTy &IRB,
                                           unsigned glb) {
  ...
  struct TCGTemp *base_tmp = get_tcg_context()->temps[glb].mem_base;
  if (unlikely(!base_tmp || temp_idx(base_tmp) != tcg_env_index))
    return nullptr;

  unsigned Off = get_tcg_context()->temps[glb].mem_offset;

  llvm::Value *Env = GetEnv(IRB);

  llvm::SmallVector<llvm::Value *, 4> Indices;
  llvm::Value *res = llvm::getNaturalGEPWithOffset(
      IRB, DL, std::make_pair(Env, CPUStateType),
  ...
  return IRB.CreateIntToPtr(
      IRB.CreateAdd(
          IRB.CreatePtrToInt(Env, WordType()),
          IRB.getIntN(WordBits(), Off)),
      GlbTy->getPointerTo());
}

template <bool MT, bool MinSize>
llvm::Value *llvm_t<MT, MinSize>::BuildCPUStatePointer(IRBuilderTy &IRB,
                                                       llvm::Value *Env,
                                                       unsigned glb) {
  assert(glb < tcg_num_globals);
  assert(glb != tcg_env_index);
            ...
            for (unsigned i = 0; i < glbv.size(); ++i) {
              ...
              if (glb == tcg_stack_pointer_index) {
                llvm::Value *ReturnedSP = IRB.CreateExtractValue(
                    ...
                    get_tcg_context()->temps[glb].name + std::string("_"));
          ...
          assert(FTy->getReturnType()->isStructTy());
          {
            ...
            llvm::Value *res = std::accumulate(
                ...
                [&](llvm::Value *res, llvm::Value *Val) -> llvm::Value * {
                  std::string nm = std::string("_ret_") + get_tcg_context()->temps[CallConvRetArray.at(j)].name + std::string("_");
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::TranslateFunctions(void) {
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::InlineSjStubs(void) {
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::PrepareToOptimize(void) {
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::DoOptimize(void) {
  PrepareToOptimize();
...
int llvm_t<MT, MinSize>::LoadRelocationSectionPointers(void) {
  ...
  auto hunt_and_erase = [&](std::function<llvm::Instruction *(llvm::Instruction &)> HuntFunc) -> void {
    ...
    std::function<void(llvm::Instruction &)> rec = [&](llvm::Instruction &I) -> void {
      if (I.use_empty())
        ToErase.push_back(&I);
      for (unsigned Idx = 0; Idx < I.getNumOperands(); ++Idx) {
        if (auto *OpInst = llvm::dyn_cast<llvm::Instruction>(I.getOperand(Idx))) {
    ...
    for (llvm::Function &F : *Module) {
      for (llvm::BasicBlock &BB : F) {
        for (llvm::Instruction &I : BB) {
          if (llvm::Instruction *Erase = HuntFunc(I)) {
            rec(*Erase);
          }
        }
      }
  ...
  llvm::ConstantInt *matched_Addend1 = nullptr;
  llvm::ConstantInt *matched_Addend2 = nullptr;
  llvm::Value *matched_Value1 = nullptr;
  llvm::Value *matched_Value2 = nullptr;
  llvm::Value *matched_Value3 = nullptr;

  auto reset = [&](void) -> void {
    matched_Addend1 = matched_Addend2 = nullptr;
  ...
  //
  // hunt for loads and stores
  //
  hunt_and_erase ([&](llvm::Instruction &I) -> llvm::Instruction * {
    unsigned PointerOperandIdx = ~0u;

    llvm::LoadInst *LI = nullptr;
    llvm::StoreInst *SI = nullptr;
    bool Match = false;

    if (LI = llvm::dyn_cast<llvm::LoadInst>(&I)) {
      PointerOperandIdx = 0;

      reset();
      Match = llvm::PatternMatch::match(&I, llvm::PatternMatch::m_Load(PtrPattern1));
      if (!Match) {
        reset();
        Match = llvm::PatternMatch::match(&I, llvm::PatternMatch::m_Load(PtrPattern2));
      }
    } else if (SI = llvm::dyn_cast<llvm::StoreInst>(&I)) {
      PointerOperandIdx = 1;

      reset();
      Match = llvm::PatternMatch::match(&I, llvm::PatternMatch::m_Store(llvm::PatternMatch::m_Value(matched_Value3), PtrPattern1));
      if (!Match) {
        reset();
        Match = llvm::PatternMatch::match(&I, llvm::PatternMatch::m_Store(llvm::PatternMatch::m_Value(matched_Value3), PtrPattern2));
      }
    } else {
      return nullptr;
    }
    ...
    llvm::Value *const Ptr = I.getOperand(PointerOperandIdx);
    llvm::Constant *const SectPtr = llvm::ConstantExpr::getIntToPtr(
        SectionPointer(Addr), Ptr->getType());

    llvm::Constant *AddrReplacement = nullptr;
    if (matched_Value1 == SectionsTop()) {
      AddrReplacement = SectionPointer(Addr);
    } else {
      AddrReplacement =
          matched_Addend1
    ...
    llvm::Value *PtrReplacement =
        llvm::ConstantExpr::getIntToPtr(AddrReplacement, Ptr->getType());
    ...
    if (LI) {
      ...
      if (matched_Value1 == SectionsTop() &&
          (it = ConstantRelocations.find(Addr)) != ConstantRelocations.end() &&
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::InternalizeSections(void) {
  if (SectsGlobal)
...
template <bool MT, bool MinSize>
bool llvm_t<MT, MinSize>::shouldExpandOperationWithSize(llvm::Value *Size) {
#if 0
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::ReplaceAllRemainingUsesOfConstSections(void) {
  if (!ConstSectsGlobal)
    return 0;
...
int llvm_t<MT, MinSize>::RenameFunctionLocals(void) {
  ...
  for (llvm::User *U : EnvGlobal->users()) {
    uint8_t glb = 0xff;
    ...
    const char *nm = get_tcg_context()->temps[glb].name;
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::ForceCallConv(void) {
  if (!IsCOFF)
    return 0;
...
static Value *buildGEP(jove::IRBuilderTy &IRB,
...
static Value *getNaturalGEPWithType(jove::IRBuilderTy &IRB,
                                    const DataLayout &DL,
                                    std::pair<Value *, Type *> BasePtr,
...
static Value *getNaturalGEPWithOffset(jove::IRBuilderTy &IRB,
                                      const DataLayout &DL,
                                      std::pair<Value *, Type *> Ptr,
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::TranslateBasicBlock(TranslateContext &TC) {
  auto &GlobalAllocaArr = TC.GlobalAllocaArr;
  ...
  auto set = [&](llvm::Value *V, unsigned glb) -> void {
    ...
    if (!Ptr) {
      IRBuilderTy tmpIRB(state.for_function(f).F->getEntryBlock().getTerminator());
  ...
  auto get = [&](unsigned glb) -> llvm::Value * {
    switch (glb) {
    case tcg_env_index:
      return IRB.CreatePtrToInt(GetEnv(IRB), WordType());
#if defined(TARGET_X86_64) || defined(TARGET_I386)
    ...
    //
    {
      ...
      QTAILQ_FOREACH(op, &s->ops, link) {
        ...
        for (int i = 0; i < nb_iargs; ++i) {
          ...
          {
            IRBuilderTy tmpIRB(&state.for_function(f).F->getEntryBlock().front());
  ...
  case TERMINATOR::INDIRECT_CALL: {
    ...
    if (options.ABICalls && IsABICall)
    {
      ...
      assert(Ret->getType()->isStructTy());
      {
        llvm::Value *X = IRB.CreateExtractValue(Ret, 0, get_tcg_context()->temps[CallConvRetArray.at(0)].name + std::string("_"));
        llvm::Value *Y = IRB.CreateExtractValue(Ret, 1, get_tcg_context()->temps[CallConvRetArray.at(1)].name + std::string("_"));
      ...
      assert(Ret->getType()->isIntegerTy(128));
      {
        llvm::Value *X =
            IRB.CreateTrunc(Ret, IRB.getInt64Ty(),
                            get_tcg_context()->temps[CallConvRetArray.at(0)].name + std::string("_"));

        llvm::Value *Y = IRB.CreateTrunc(
            IRB.CreateLShr(Ret, IRB.getIntN(128, 64)), IRB.getInt64Ty(),
            get_tcg_context()->temps[CallConvRetArray.at(1)].name + std::string("_"));
        ...
        llvm::BasicBlock *DoCallB = DynTargetsDoCallBVec[i];
        {
          ...
          if (foreign) {
...

            {
              ...
              ArgVec.push_back(BuildCPUStatePointer(IRB, GetEnv(IRB),
                                                    tcg_stack_pointer_index));
          ...
          if (foreign) {
            ...
            assert(Ret->getType()->isStructTy());
            {
              llvm::Value *X = IRB.CreateExtractValue(
                  Ret, 0, get_tcg_context()->temps[CallConvRetArray.at(0)].name + std::string("_"));
              llvm::Value *Y = IRB.CreateExtractValue(
                  Ret, 1, get_tcg_context()->temps[CallConvRetArray.at(1)].name + std::string("_"));
            ...
            for (unsigned j = 0; j < CallConvRetArray.size(); ++j) {
              llvm::Value *X = IRB.CreateExtractValue(
                  Ret, j, get_tcg_context()->temps[CallConvRetArray.at(j)].name + std::string("_"));
...
#error
  ...
  case TERMINATOR::CONDITIONAL_JUMP: {
    ...
    if (is1) {
      ...
      auto succ = ICFG.template adjacent_n<2>(bb);
      succ1 = succ[0];
      succ2 = succ[1];
    }
...

    {
      ...
      llvm::Value *retVal = std::accumulate(
          ...
          [&](llvm::Value *res, unsigned glb) -> llvm::Value * {
            std::string nm = std::string("_ret_") + get_tcg_context()->temps[glb].name + std::string("_");
...
template <bool MT, bool MinSize>
std::string llvm_t<MT, MinSize>::dyn_target_desc(dynamic_target_t IdxPair) {
  struct {
    binary_index_t BIdx;
    function_index_t FIdx;
  } DynTarget;
  ...
  const function_t &f = b.Analysis.Functions.at(DynTarget.FIdx);
...
template <bool MT, bool MinSize>
int llvm_t<MT, MinSize>::TranslateTCGOps(llvm::BasicBlock *ExitBB,
                                         IRBuilderTy &IRB,
                                         TranslateContext &TC) {
  ...
  /* special-cased global temps */
  auto get_special = [&](TCGTemp *ts) -> llvm::Value * {
  ...
  /* get pointer to temp */
  auto get_ptr = [&](TCGTemp *ts) -> llvm::Value * {
    assert(ts->kind != TEMP_CONST);
    assert(!get_special(ts));
  ...
  auto get = [&](TCGTemp *ts) -> llvm::Value * {
    ...
    if (llvm::Value *V = get_special(ts)) {
      assert(temp_idx(ts) == tcg_env_index
                 ? true
                 : V->getType()->isIntegerTy(bitsOfTCGType(ts->type)));
      return V;
    }
...
#define __out_bits() ({ 8u * tcg_type_size((TCGType)TCGOP_TYPE(op)); })

#define out_bits()                                                             \
  ({                                                                           \
    unsigned __n = __out_bits();                                               \
    assert(__n == bitsOfTCGType(s->temps[temp_idx(output_arg(0))].type));      \
    __n;                                                                       \
  })
  ...
  auto do_the_qemu_ld = [&](llvm::Value *Addr, MemOpIdx oi,
    ...
    case MO_UL:
    case MO_UQ:
      Res = IRB.CreateZExt(Res, IRB.getIntNTy(bits));
      break;
    case MO_SB:
    case MO_SW:
    case MO_SL:
      Res = IRB.CreateSExt(Res, IRB.getIntNTy(bits));
      break;
    default:
      die("Unsupported MO_SSIZE in qemu_ld");
    }
...
#define ENTRY(Name) [INDEX_op_##Name] = REF(Name),
  ...
  static constexpr unsigned SZ = 1 << 8;
...
#define TODO() goto do_todo
  ...
  nb_oargs = opc == INDEX_op_call ? TCGOP_CALLO(op) : jv_tcgopc_nb_oargs_in_def(opc);
...
#if 0
      if (IsVeryVerbose())
        llvm::errs() << llvm::formatv("insn @ {0:x}\n", Addr);
#endif
  ...
  CASE(call): {
    ...
    if (nb_oargs == 1) {
      ...
      if (dst1->type == TCG_TYPE_I32 &&
        ...
        llvm::errs() << llvm::formatv(
            "nb_oargs={0} dst1 bits {1} dst2 bits {2}\n", nb_oargs,
            bitsOfTCGType(dst1->type),
            bitsOfTCGType(dst2->type));
      }
    }
...
*/
  CASE(extract): {
    ...
    set(IRB.CreateAnd(IRB.CreateLShr(value, IRB.getIntN(bits, start)),
                      IRB.getInt(llvm::APInt::getLowBitsSet(bits, length))),
        output_arg(0));
    BREAK();
  }
...
*/
  CASE(ctz): {
    ...
    set(IRB.CreateSelect(
            IRB.CreateICmpNE(X, llvm::ConstantInt::get(X->getType(), 0)),
            IRB.CreateBinaryIntrinsic(llvm::Intrinsic::cttz, X, IRB.getTrue()), Y),
        output_arg(0));
    BREAK();
  }
...
*/
  CASE(rotl): {
    ...
    llvm::Value *mask = IRB.getIntN(bits, bits-1);
