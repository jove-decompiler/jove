...
The C code `jove decompile` produces for non-trivial machine code instructions does **not** involve `llvm-cbe`. [It originates from QEMU's sources practically verbatim, via a custom plugin while compiling QEMU with `clang`](https://github.com/aleden/carbon-copy). Luckily QEMU is nearly completely written in C.
