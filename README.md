...
We only demand that a tiny subset of the [gigantic LLVM language specification](https://llvm.org/docs/LangRef.html) be handled by `llvm-cbe`. This proper subset produced by `jove llvm`, which translates the straightforward [TCG](https://github.com/qemu/qemu/blob/master/docs/devel/tcg-ops.rst) (QEMU's intermediate language) instructions into LLVM instructions, is much of the time essentially a one-to-one mapping.
