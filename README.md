...
2. `jove ipt` is a custom `Intel Processor Trace`-based tracer[^3]. Crucially, unlike `jove bootstrap`, its overhead is extremely low, which makes it suitable for demanding interactive applications (e.g. games). It is supported on all platforms. Unfortunately, there is commonly particularly troublesome "errata" for old CPUs.
