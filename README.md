...
# JOVE README
`jove` is a decompiler for C programs built on top of [QEMU](https://www.qemu.org/) and [LLVM](https://llvm.org/docs/LangRef.html). By observing control-flow at runtime, it is capable of perfectly disassembling machine-code binaries. The output is such that it can be modified, recompiled, and, executed. Currently, the C code it produces is generated by `llvm-cbe`. It targets userland linux on `i386`/`x86_64`/`mips`/`mipsel`/`mips64el`/`aarch64`, and Windows executables under WINE on `x86`/`x64`.
