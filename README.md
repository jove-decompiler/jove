...
The classic abstract interpretation for which there are open-source implementations widely available (e.g. [BAP](https://github.com/BinaryAnalysisPlatform/bap)) is Value Set Analysis (VSA). Basically it's just reasoning with strided intervals. Eventually `jove` may acquire an implementation of VSA.
...
We only demand that a tiny subset of the [gigantic LLVM language specification](https://llvm.org/docs/LangRef.html) to be handled by `llvm-cbe`. This proper subset is produced by `jove llvm`, which translates the straightforward [TCG](https://github.com/qemu/qemu/blob/master/docs/devel/tcg-ops.rst) (QEMU's intermediate language) instructions into LLVM instructions. Much of the time this is essentially a one-to-one mapping.
