...
class Off2Addr : public JVTool<ToolKind::CopyOnWrite> {
  struct Cmdline {
    cl::opt<std::string> Off;
    cl::opt<std::string> Binary;
    cl::alias BinaryAlias;

    Cmdline(llvm::cl::OptionCategory &JoveCategory)
        : Off(cl::Positional, cl::Required, cl::desc("File Offset"),
...
public:
  Off2Addr() : opts(JoveCategory) {}

  int Run(void) override;
};

JOVE_REGISTER_TOOL("off2addr", Off2Addr);

int Off2Addr::Run(void) {
  //
  ...
  uint64_t Off = strtoull(opts.Off.c_str(), nullptr, 0x10);

  llvm::outs() << llvm::formatv("{0:x}\n", B::va_of_offset(Bin.get(), Off));
  return 0;
}
