...
public:
  ...
  llvm::Value *CPUStateGlobalPointer(llvm::IRBuilderTy &, unsigned glb);
...
llvm::Value *LLVMTool::CPUStateGlobalPointer(llvm::IRBuilderTy &IRB,
                                             unsigned glb) {
...
int LLVMTool::TranslateTCGOp(TCGOp *op,
  ...
  auto get = [&](TCGTemp *ts) -> llvm::Value * {
    ...
    llvm::LoadInst *LI = IRB.CreateLoad(TypeOfTCGType(ts->type), get_ptr(ts));
    LI->setMetadata(llvm::LLVMContext::MD_alias_scope, AliasScopeMetadata);
    return LI;
  };
  ...
  auto set = [&](llvm::Value *V, TCGTemp *ts) -> void {
    ...
    if (V->getType()->isPointerTy()) {
      ...
      V = IRB.CreateIntCast(V, TypeOfTCGType(ts->type), false);
    }
  ...
  const int nb_oargs = opc == INDEX_op_call ? TCGOP_CALLO(op) : jv_tcgopc_nb_oargs_in_def(opc);
  const int nb_iargs = opc == INDEX_op_call ? TCGOP_CALLI(op) : jv_tcgopc_nb_iargs_in_def(opc);
  const int nb_cargs = jv_tcgopc_nb_cargs_in_def(opc);
...
#define __ARITH_OP(opc_name, LLVMOp, bits)                                     \
  case opc_name: {                                                             \
    ...
    /* pointer. */                                                             \
    if (opc_name == INDEX_op_add_i64 && WordBits() == 32) {                    \
