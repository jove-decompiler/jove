...
#define emusp           __jove_env.regs[R_ESP]
  ...
  for (unsigned BIdx = 0; BIdx < _JOVE_MAX_BINARIES; ++BIdx) {
    ...
    for (unsigned FIdx = 0; fns[2 * FIdx]; ++FIdx) {
      ...
      // syscall
      //
      if (((uint8_t *)saved_retaddr)[0] == 0x48 &&
          ((uint8_t *)saved_retaddr)[1] == 0xc7 &&
          ((uint8_t *)saved_retaddr)[2] == 0xc0 &&
          ((uint8_t *)saved_retaddr)[3] == 0x0f &&
          ((uint8_t *)saved_retaddr)[4] == 0x00 &&
          ((uint8_t *)saved_retaddr)[5] == 0x00 &&
          ((uint8_t *)saved_retaddr)[6] == 0x00 &&
          ((uint8_t *)saved_retaddr)[7] == 0x0f &&
        ...
        {
          char buff[65];
          _uint_to_string(((uint8_t *)saved_retaddr)[i], buff, 0x10);
...

      {
        ...
        _newsp -= sizeof(uintptr_t); /* return address on the stack */
