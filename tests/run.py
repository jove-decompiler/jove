...
scp(jove_rt_path, '/lib/libjove_rt.so')
...
def run_tests():
  for test in args.tests:
    ...
    for variant in ["exe", "pic"]:
      ...
      for i in range(0, 2):
        for input_args in test_inputs:
          ssh(["jove", "loop", "-x", "--optimize", "--connect", "%s:%d" % (iphost, jove_server_port), test_guest_path] + input_args)

      for input_args in test_inputs:
        p1 = ssh_command([test_guest_path] + input_args, text=True)
        p2 = ssh_command(["jove", "loop", "-x", "--optimize", "--connect", "%s:%d" % (iphost, jove_server_port), test_guest_path] + input_args, text=True)
