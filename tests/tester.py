...
class JoveTester:
  ...
  def ssh_command(self, command, text=True):
    return subprocess.run(['autossh', '--'] + self.ssh_common_args + ['-p', str(self.guest_ssh_port), 'root@localhost'] + command, capture_output=True, text=text)

  def ssh(self, command):
    return subprocess.run(['autossh', '--'] + self.ssh_common_args + ['-p', str(self.guest_ssh_port), 'root@localhost'] + command)
