...
#define _DEFINE_INTERPROCESS_MUTEX(Name)                                       \
  Name(*segment.find_or_construct<boost::interprocess::interprocess_mutex>(    \
      #Name)())

#define _DECLARE_SHARED_MEMORY_MAP(Name, KeyType, MappedType)                  \
  typedef boost::hash<KeyType> Name##_Hasher;                                  \
  ...
  typedef boost::interprocess::allocator<                                      \
      ...
      Name##_AllocType;                                                        \
  typedef boost::unordered_map<Name##_KeyType, Name##_MappedType,              \
                               Name##_Hasher, std::equal_to<Name##_KeyType>,   \
                               Name##_AllocType>                               \
      Name##_Type;                                                             \
...
#define _DECLARE_SHARED_MEMORY_VECTOR(Name, Type)                              \
  typedef Type Name##_ValueType;                                               \
  typedef boost::interprocess::allocator<                                      \
      Name##_ValueType,                                                        \
      boost::interprocess::managed_shared_memory::segment_manager>             \
      Name##_AllocType;                                                        \
  typedef boost::interprocess::vector<Name##_ValueType, Name##_AllocType>      \
      Name##_Type;                                                             \
  Name##_AllocType Name##_alloc;                                               \
  Name##_Type &Name;

#define _DEFINE_SHARED_MEMORY_VECTOR(Name)                                     \
  Name##_alloc(segment.get_segment_manager()),                                 \
  Name(*segment.find_or_construct<Name##_Type>(#Name)(Name##_alloc))
