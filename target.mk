...
$(build_dir)/obj2llvmdump-$(_TARGET_NAME): $(build_dir)/obj2llvmdump-$(_TARGET_NAME).2.bc
	$(llvm_dir)/bin/clang++ -o $@ $< -O3 -flto -fPIC $(shell $(llvm_dir)/bin/llvm-config --libs object) $(shell $(llvm_dir)/bin/llvm-config --ldflags) -lglib-2.0 -pthread -lcurses

$(build_dir)/obj2llvmdump-$(_TARGET_NAME).5.bc: $(build_dir)/obj2llvmdump-$(_TARGET_NAME).4.bc
	@echo BCLINK $(notdir $@ $^)
	$(llvm_dir)/bin/opt -o $@ -globaldce $<

$(build_dir)/obj2llvmdump-$(_TARGET_NAME).4.bc: $(build_dir)/obj2llvmdump-$(_TARGET_NAME).3.bc
	$(build_dir)/llknife -o $@ -i $< --only-external-regex 'main'

$(build_dir)/obj2llvmdump-$(_TARGET_NAME).3.bc: $(build_dir)/obj2llvmdump-$(_TARGET_NAME).2.bc $(boost_dir)/lib/libboost_system.a $(boost_dir)/lib/libboost_program_options.a
	$(llvm_dir)/bin/clang++ -flto -emit-llvm -o $@ $^
...
$(build_dir)/obj2llvmdump-$(_TARGET_NAME).0.bc: $(build_dir)/qemu-$(_TARGET_NAME).bc $(build_dir)/qemutcg-$(_TARGET_NAME).bc $(build_dir)/obj2llvmdump-$(_TARGET_NAME).bc
	@echo BCLINK $(notdir $@ $^)
